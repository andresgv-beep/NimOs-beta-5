<script>
  ...
  // ── Estado ──
  let shares = [];
  let openFiles = [];       // { name, path, share, content, modified, lang }
  let activeIdx = -1;
  ...
  const LANGS = {
    py:'Python', js:'JavaScript', ts:'TypeScript', svelte:'Svelte',
    go:'Go', c:'C', cpp:'C++', rs:'Rust', sh:'Shell',
    md:'Markdown', json:'JSON', yaml:'YAML', yml:'YAML',
    html:'HTML', css:'CSS', txt:'Texto', nim:'NimOS',
  };

  const LANG_COLORS = {
    py:'#60a5fa', js:'#fbbf24', ts:'#60a5fa', go:'#4ade80',
    c:'#f97316', cpp:'#f97316', rs:'#f97316', svelte:'#f97316',
    md:'#c084fc', json:'#e879f9', sh:'#4ade80', txt:'',
  };

  function getExt(name) {
    const d = name.lastIndexOf('.'); return d >= 0 ? name.slice(d+1).toLowerCase() : 'txt';
  }
  ...
  // ── Árbol de proyecto ──
  async function loadTree(share, path) {
    treeLoading = true;
    try {
  ...
  async function openProject(share) {
    ...
    projectTree = await loadTree(share, '/');
  }
  ...
  async function openFile(share, path, name) {
    ...
    try {
      ...
      openFiles = [...openFiles, { name, path, share, content, modified: false, lang: getLang(name) }];
  ...
  async function saveFile() {
    ...
    try {
      ...
      // path sin el nombre del archivo
      ...
      if (res.ok) {
        openFiles[activeIdx].modified = false;
        openFiles = [...openFiles];
      }
    ...
    {#if openFiles.length > 0}
      ...
      {#each openFiles as f, i}
        ...
        <div class="sb-item" class:active={i === activeIdx} on:click={() => { activeIdx = i; activeLine = 1; }}>
          <svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" style="width:13px;height:13px;flex-shrink:0;opacity:{i===activeIdx?1:0.5}"><path d="M14 2H6a2 2 0 0 0-2 2v16a2 2 0 0 0 2 2h12a2 2 0 0 0 2-2V8z"/><polyline points="14 2 14 8 20 8"/></svg>
          <span class="sb-item-name">{f.name}</span>
          {#if f.modified}<div class="sb-dot"></div>{/if}
          <span class="sb-ext" style="background:rgba(255,255,255,0.06);color:{getLangColor(f.name)}">{getExt(f.name)}</span>
        </div>
      {/each}
    {/if}

    {#if projectShare}
      <div class="sb-section" style="margin-top:12px">Proyecto</div>
      {#each projectTree as item}
        {#if item.isDirectory}
          <!-- svelte-ignore a11y_click_events_have_key_events -->
          <!-- svelte-ignore a11y_no_static_element_interactions -->
          <div class="sb-folder" class:open={expandedFolders.has(item.name)} on:click={() => toggleFolder(item.name)}>
            <svg class="arrow" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2.5" stroke-linecap="round" style="width:11px;height:11px;flex-shrink:0;transition:transform .15s"><polyline points="9 18 15 12 9 6"/></svg>
            <span style="font-size:12px">📁</span>
            <span class="sb-folder-name">{item.name}</span>
          </div>
        {:else}
          <!-- svelte-ignore a11y_click_events_have_key_events -->
          <!-- svelte-ignore a11y_no_static_element_interactions -->
          <div class="sb-item sb-tree-file" on:click={() => openFile(projectShare, (projectPath === '/' ? '/' : projectPath) + '/' + item.name, item.name)}>
            <svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" style="width:12px;height:12px;flex-shrink:0;opacity:.5"><path d="M14 2H6a2 2 0 0 0-2 2v16a2 2 0 0 0 2 2h12a2 2 0 0 0 2-2V8z"/><polyline points="14 2 14 8 20 8"/></svg>
  ...
  <div class="inner-wrap">
    <div class="inner">
      ...
      <div class="inner-titlebar">
        <!-- svelte-ignore a11y_click_events_have_key_events -->
        <!-- svelte-ignore a11y_no_static_element_interactions -->
        <button class="icon-btn" on:click|stopPropagation={() => sidebarOpen = !sidebarOpen} title="Toggle sidebar">
        ...
        <div class="menu-wrap" on:click|stopPropagation>
          <!-- svelte-ignore a11y_click_events_have_key_events -->
          <!-- svelte-ignore a11y_no_static_element_interactions -->
          <button class="icon-btn" on:click={() => showMenu = !showMenu} title="Más opciones">
          ...
          {#if showMenu}
            <div class="dropdown">
              <!-- svelte-ignore a11y_click_events_have_key_events -->
              <!-- svelte-ignore a11y_no_static_element_interactions -->
              <div class="drop-item" on:click={() => { showMenu = false; }}>
              ...
              <!-- svelte-ignore a11y_click_events_have_key_events -->
              <!-- svelte-ignore a11y_no_static_element_interactions -->
              <div class="drop-item" on:click={() => { saveFile(); showMenu = false; }}>
        ...
        <!-- Guardar -->
        <button class="icon-btn save" on:click={saveFile} disabled={!activeFile?.modified || saving} title="Guardar (Ctrl+S)">
          {#if saving}
      ...
      {#if activeFile}
        <div class="editor-wrap">
          ...
          <div class="line-numbers">
            {#each lines as _, i}
              <div class="ln" class:active={i+1 === activeLine}>{i+1}</div>
            {/each}
          </div>

          <!-- Código con textarea superpuesta -->
          <div class="code-wrap">
            <!-- Capa de highlight (solo visual) -->
            ...
            <!-- Textarea editable transparente encima -->
            <textarea
              class="code-textarea"
      ...
      <div class="statusbar">
        ...
        {#if activeFile}
          <span class="st-sep">·</span>
          <span style="color:var(--text-3);font-family:'DM Mono',monospace;font-size:10px">{activeFile.share}{activeFile.path}</span>
          <div class="st-right">
...
<style>
  ...
  .sb-folder { display:flex; align-items:center; gap:6px; padding:5px 8px; margin:0 4px; border-radius:6px; font-size:11px; color:var(--text-2); cursor:pointer; transition:all .12s; white-space:nowrap; }
  .sb-folder:hover { color:var(--text-1); }
  .sb-folder.open .arrow { transform:rotate(90deg); }
  ...
  .icon-btn.save:disabled { opacity:.4; cursor:not-allowed; }
  ...
  :global(.op)  { color:rgba(255,255,255,0.55); }
  ...
  .empty-desc { font-size:11px; color:var(--text-3); text-align:center; max-width:240px; }
